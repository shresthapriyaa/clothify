...
export default function BagsPage() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch("/api/products?category=bag");
      ...
      // Filter for bag products if API doesn't filter
      const bagProducts = productsArray.filter(
        (p) => p.category.name.toLowerCase() === "bag"
      );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-100 py-12">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-gray-600">Explore our collection of bags</p>
        </div>
      </div>
